...
def compute_C_D_uniform(
    ...
    if quad_rule == 1:
        ...
        if halfN > 0:
            ...
            # C matrix (special case i=2 to avoid 0 denominator at r0=0)
            with np.errstate(divide='ignore', invalid='ignore'):
                term2_C2 = 4 * 1.0 * (0.5 ** (-k)) * f_pos[:, 1:2]
                term3_C2 = 2.0 * (1.0 ** (-k)) * f_pos[:, 2:3]
                C[:halfN, 1:2] = (delta**2 / (6 * k)) * (term2_C2 + term3_C2)

            # C matrix (main part i=3..M-1)
            if M > 3:
                i_c = np.arange(3, M)
                i_c_out = i_c - 1
                i_c_b = i_c[None, :]
